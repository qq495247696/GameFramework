...
void LoadModel(char* FileName, DX11_MODEL* Model,DirectXAPI* api)
{
	...
	// 頂点バッファ生成
	{
		...
		api->GetDevice()->CreateBuffer(&bd, &sd, &Model->VertexBuffer);
	}
	...
	// インデックスバッファ生成
	{
		...
		api->GetDevice()->CreateBuffer(&bd, &sd, &Model->IndexBuffer);
	}
	...
	// サブセット設定
	{
		...
		for (unsigned short i = 0; i < model.SubsetNum; i++)
		{
			...
			D3DX11CreateShaderResourceViewFromFile(api->GetDevice().Get(),
...
void DrawModel(DX11_MODEL* Model,DirectXAPI *api)
{
	...
	UINT offset = 0;
	api->GetDeviceContext()->IASetVertexBuffers(0, 1, &Model->VertexBuffer, &stride, &offset);

	// インデックスバッファ設定
	api->GetDeviceContext()->IASetIndexBuffer(Model->IndexBuffer, DXGI_FORMAT_R16_UINT, 0);
	...
	{
		// マテリアル設定
		api->SetMaterial(Model->SubsetArray[i].Material.Material);

		// テクスチャ設定
		if (Model->SubsetArray[i].Material.Material.noTexSampling == 0)
			api->GetDeviceContext()->PSSetShaderResources(0, 1, &Model->SubsetArray[i].Material.Texture);

		// ポリゴン描画
		api->GetDeviceContext()->DrawIndexed(Model->SubsetArray[i].IndexNum, Model->SubsetArray[i].StartIndex, 0);
	}
}
